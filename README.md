...
기존 회원가입이나 비밀번호 변경 등 form register를 구현할떄마다 usestate를 사용하여 
error, 상태값, 조건, 서버 통신값 관리 등 코드가 길어져서 가독성및 유지보수가 떨어지는 상황이 발생했습니다.  
**react-hook-form**을 사용하여 가독성을 높이고 유지보수를 높였습니다.  

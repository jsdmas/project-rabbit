...
const EditThread = () => {
  ...
  const { data: response, isLoading } = useQuery<IResponse>(
    ['thread', threadid],
    () => fetchMainTextThread(threadid),
    { onError: (error) => errorMessage(error), staleTime: 1000 * 30 },
  );
  const { mutate: editThread, isLoading: editRequest } = useMutation(
    (postData: IpostData) => updateThread(postData, threadid),
    {
      onSuccess: () => {

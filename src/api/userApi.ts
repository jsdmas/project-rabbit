...
export const loginStatus = async () => {
  ...
  try {
    const {
      data: { loginState, loginUserId, loginUserSnsId },
    } = await axios.get('/api/auth/check-login-status', { withCredentials: true });
...
export const ChangeOfPassword = async ({ currentPW, changePW, loginUserId }: IPasswordChange) => {
  ...
  try {
    const { data } = await axios.patch(
      `/api/change-password`,
      { currentPW, changePW, loginUserId },
      { withCredentials: true },
    );
    response = data;
  } catch (error) {
